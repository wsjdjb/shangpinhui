<template>
  <div class="detail">
    ...
    <section class="con">
      <!-- 导航路径区域 -->
      <div class="conPoin">
        <span v-if="this.categoryView.category1Name">{{categoryView.category1Name}}</span>
        <span v-if="this.categoryView.category2Name">{{categoryView.category2Name}}</span>
        <span v-if="this.categoryView.category1Name">{{categoryView.category3Name}}</span>
      </div>
      <!-- 主要内容区域 -->
      <div class="mainCon">
        <!-- 左侧放大镜区域 -->
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(sale,index) in this.spuSaleAttrList" 
                :key="sale.id">
                <dt class="title" v-if="sale">选择{{sale.saleAttrName}}</dt>
                <dd changepirce="0" :class="attr.isChecked==1?'active':''" 
                v-for="(attr,index) in sale.spuSaleAttrValueList" 
                :key="attr.id"
                @click="changeChecked(sale.spuSaleAttrValueList,index)"
                >{{attr.saleAttrValueName}}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="this.num" @change="changeNum(`change`,this.num*1)">
                <a  class="plus" @click="changeNum(`plus`,1)">+</a>
